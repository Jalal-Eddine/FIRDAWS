<?php
  ...
  // * preg_match(string $pattern , string $subject) is helpful, uses a regular expression
  ...
  function has_unique_page_menu_name($menu_name, $current_id="0") {
    global $db; //to connect to the database the variable is defined in initialize.php

    $sql = "SELECT * FROM pages ";
    $sql .= "WHERE menu_name='" . db_escape($db, $menu_name) . "' ";
    $sql .= "AND id != '" . db_escape($db, $current_id) . "'";//see database.php

    $page_set = mysqli_query($db, $sql);//return all pages where id=$current_id
    $page_count = mysqli_num_rows($page_set);//
    mysqli_free_result($page_set);

    return $page_count === 0;//?:return FALSE if $page_count !=== 0, TRUE if it's === 0.
  }
  ...
  function has_unique_username($username, $current_id="0") {
    ...
    return $admin_count === 0;//?:return FALSE if $admin_count !=== 0, TRUE if it's === 0.
  }
